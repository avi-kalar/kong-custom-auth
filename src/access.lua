local JSON = require "kong.plugins.kong-custom-auth.json"
...
function _M.execute(conf)
  ...
  local name = "[kong-custom-auth] "
  local ok, err
  local parsed_url = parse_url(conf.url)
  local host = parsed_url.host
  local port = tonumber(parsed_url.port)
  local payload = _M.compose_get_request(parsed_url)
  ...
  if status_code == 401 then
    ...
    return kong.response.exit(403, "Forbidden Access")
  end
...
function _M.compose_get_request(parsed_url)
    ...
    local payload_body = [[{"headers":]] .. raw_json_headers .. [[}]]
